...
### Task 1: Implement Missing Services
- Implement the `ShelterService` class that fulfills the `IShelterService` interface
...
### Task 2: Create Missing Controllers
- Create a `SheltersController` with CRUD operations
- Create an `AdoptionApplicationsController` with CRUD operations
- Implement proper authorization (only Admin and ShelterStaff can manage shelters)
...
- Add response caching for the `GetAllPets` and `GetAllShelters` endpoints
