...
namespace AdoptAPet.Tests
{
    public class Task3_FilteringPaginationTests : ApiTestBase
    {
        ...
        [Fact]
        public async Task GetPets_WithBreedFilter_ReturnsFilteredPets()
        {
            // First get a breed from the database to use for filtering
            var pet = await _dbContext.Pets.FirstOrDefaultAsync();
            var breed = pet?.Breed;

            // Skip test if no pets with breeds are found
            ...
            // Act
            var response = await _client.GetAsync($"/api/Pets?breed={breed}");
            ...
            result.Should().NotBeNull();
            result!.Items.Should().NotBeNull();
            result.Items.Should().AllSatisfy(pet => pet.Breed.Should().Be(breed));
        }
